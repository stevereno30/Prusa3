...
module bearing_guide_inner(){
    ...
    difference(){
        cylinder(r=idler_bearing[0] / 2 + single_wall_width * 1, h=height - (max( idler_bearing[1], belt_width) - idler_bearing[1]) / 2);
        translate([0, 0, -1])
...
module bearing_guide_outer(){
    difference(){
        cylinder(r=idler_bearing[0] / 2 + single_wall_width + 1, h=height);
        translate([0,0,-1])
            cylinder(r=idler_bearing[0] / 2 + single_wall_width * 1.05, h=height + 2);
    }
